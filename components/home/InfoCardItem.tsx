  ...
  const inner = (
    <>
      <div className="flex min-w-0 items-center gap-2 sm:gap-3">
        <Icon
          className="h-4 w-4 shrink-0 text-primary transition-colors duration-200 group-hover:text-white sm:h-5 sm:w-5"
          strokeWidth={2}
          aria-hidden="true"
        />
        <h3 className="truncate text-base font-black tracking-tight text-primary transition-colors duration-200 group-hover:text-white sm:text-[22px]">
          {item.title}
        </h3>
      </div>
      <span
        aria-hidden="true"
        className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-white text-primary transition-colors duration-200 group-hover:bg-white group-hover:text-primary sm:h-12 sm:w-12"
      >
        <ArrowRight
