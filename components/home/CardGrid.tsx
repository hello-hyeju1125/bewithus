...
export default function CardGrid() {
  ...
  const timetable = items.slice(0, 4) as Array<{
    title: string;
    subtitle?: string;
    href: string;
  }>;
        ...
        {timetable.map((item, idx) => {
          ...
          return (
            <WidgetActionLink
              key={item.title}
              href={item.href}
              className="group relative flex h-full min-h-[108px] flex-col justify-start rounded-card border-2 border-transparent bg-primary-50 px-5 py-2.5 outline-none transition-all duration-200 ease-out hover:-translate-y-1 hover:border-primary hover:bg-accent-500 hover:shadow-[0_12px_24px_-8px_rgba(34,41,93,0.25)] focus-visible:ring-2 focus-visible:ring-primary sm:min-h-[150px] sm:px-6 sm:py-4"
            >
              <Icon
