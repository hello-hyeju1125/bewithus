...
import { updateTimetableSubjectOrderAction } from "../actions";
...
export default function CourseSubjectSortableList({
  ...
  function onDragEnd(e: DragEndEvent) {
    ...
    const updates = next.map((subject, order_index) => ({
      subject,
      order_index,
    }));
    startTransition(async () => {
      const res = await updateTimetableSubjectOrderAction(updates);
      if (!res.ok) {
  ...
  if (items.length === 0) {
    return (
      <p className="rounded-card border border-dashed border-neutral-300 bg-neutral-50 px-4 py-6 text-center text-[13px] text-neutral-500">
        등록된 과목이 없습니다. 상세 강의를 등록하면 과목이 표시됩니다.
      </p>
    );
  }
